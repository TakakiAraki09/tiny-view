...
**Version:** v0.2
**Status:** Implemented — 実装 crate v1.1.0 に追従する living document
...
> 本ドキュメントは MVP 仕様策定時の版数 (v0.2) を保持しつつ、出荷済み実装 (crate v1.1.0) の実態に追従する。実装と本文がズレた場合は実装を正とし本文を更新する。
...
どの候補も存在しなければ、後方互換のため末尾の `~/.tinyview/` をそのまま最終 fallback として返す（実装 `src/config.rs` `config_root()` と一致）。
...
- CSP `<meta>` 注入（permission が無い raw では skip する。詳細は §19.5）
...
window.__TINYVIEW__ = {
  ...
  title: string, // 現状は定数 "tinyview" 固定（入力/パスからは導出しない）
  path: string | null, // file 入力時のみ。stdin / inline は null
};
```

> 注: `title` は現時点では実装 (`src/main.rs`) で定数 `"tinyview"` 固定であり、WebView ウィンドウのタイトルバー (`.with_title("tinyview")`) も同じ定数を使う。入力ファイル名やコンテンツからは導出しない。
...
| Watch (`--watch`) | `notify-debouncer-mini`（常時リンク、feature gate なし）                                                                                            | 起動時 init しないため raw path への runtime cost ~0。バイナリ +~200KB。`notify` は `notify_debouncer_mini::notify` 再エクスポート経由で transitive 利用し二重リンクを回避 |
| Template engine   | **なし**                                                                                                                                            | `str::replace` 1回で完結                                                                                            |
| Menu bar (macOS)  | `muda`（macOS target 限定 / `default-features = false`）                                                                                            | 標準 macOS ショートカット (Cmd+Q/W/M, Ctrl+Cmd+F, Cmd+C/V/X/A) を NSMenu 経由で供給。Windows/Linux ビルドには非リンク。`src/menu.rs` 参照 |
...
**MSRV:** `rust-version = 1.88`。これは TinyView 自身のコードではなく、`wry`（ネイティブ WebView core）と transitive な `time-core` が宣言する下限に追従したもの。native WebView 要件 (CLAUDE.md) 上 `wry` は外せないため MSRV は依存実態で決まる。`msrv` CI job が同一 toolchain でビルドして検証する。

**Cargo features:** production ビルド向けの feature は存在しない。唯一の feature `e2e` は live-WebView 自己テストハーネス (`src/e2e.rs`) と JS→native IPC bridge をコンパイルするためのもので、production には **絶対に入れない**（"no native bridge" セキュリティデフォルトを担保するため bridge を feature 裏に隔離している）。
