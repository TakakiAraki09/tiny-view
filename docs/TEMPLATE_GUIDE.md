...
**fetch / XHR / WebSocket はすべて遮断**されます（meta が無い状態）。

許可経路は **template / 入力 HTML 自身による `<meta>` 宣言のみ**です。CLI フラグは
ありません。

### 6.1 template 側で宣言する `<meta name="tinyview-allow">`

template の `<head>` に次の meta を置くと、その template では
fetch / XHR / WebSocket が許可されます。
...
- meta があると runtime は CSP の `connect-src` を `'none'` から
  `https: http: ws: wss:` に緩和します（`src/webview.rs` の `build_csp()`）。

これは「この template は本質的にネットワークを必要とする」という宣言を template 自身に
持たせる仕組みです。必要権限を一番よく知っているのは template 自身なので、利用者が
毎回 CLI で指定するのではなく、template が自己宣言します。

### 6.2 許可の関係まとめ

| `<meta name="tinyview-allow" content="fetch">` | 結果 |
| :--: | ---- |
| 無し | `connect-src 'none'`（全ネットワーク遮断、デフォルト） |
| 有り | fetch 許可 |
...
# fetch を使う template の確認（template の <head> に tinyview-allow meta を置く。§6.1）
cat data.json | tinyview -t my-fetch-template
```
...
- **fetch がブロックされる**: `<head>` に meta（`<meta name="tinyview-allow" content="fetch">`）が
  無いと `connect-src 'none'` で遮断される。template に meta を付ける（§6.1）。
