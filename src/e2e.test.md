...
権限許可（fetch は HTML の `<meta name="tinyview-allow" content="fetch">`、clipboard / storage は `--allow-*` フラグ）を **実際の WebView** で検証する live self-test harness（issue #5、`e2e` feature でのみコンパイル）。
...
|検証事項|内容|備考|
|---|---|---|
|デフォルト CSP が fetch をブロック|`connect-src 'none'` 違反が発火 → `blocked`。|fetch (meta) / hard|
|meta で fetch 許可|HTML の `<meta name="tinyview-allow" content="fetch">` により違反は発火せず `allowed`。fetch 許可の唯一の経路（`--allow-fetch` フラグは廃止）。|fetch (meta) / hard|
