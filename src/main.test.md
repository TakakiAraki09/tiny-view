...
|既存フラグと共存|`--frameless --transparent` を追加しても `--width` / `--height` / `--foreground` のパースを壊さない。|リグレッションガード|
|`--allow-fetch` は受け付けない|廃止済みフラグ（PRD §19.2.1、fetch 許可は HTML の `<meta name="tinyview-allow" content="fetch">` のみ）。パースはエラーになる。|breaking change ガード|
